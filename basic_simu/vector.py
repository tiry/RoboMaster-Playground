...
## numpy allows to manipulate 1d arrays as vectors, but some features are missing and this is not efficient
## pygame vector class sucks
...
class Vector(object):
    def __init__(self, *args):
        ...
        if len(args)==0: self.values = (0,0)
            ...
            if isinstance(args[0], Vector):
                self.values = tuple( x for x in args[0].values )
            else: 
                self.values = args
    ...
    def scale(self, vector):
        """ scale one vector using the second one
        """
        if not isinstance(vector, Vector):
            raise ValueError('The scale operation requires another vector')
        scaled = tuple(a * b for a, b in zip(self, vector))
        return self.__class__(*scaled)
    
    def round(self, digits):
        """ scale one vector using the second one
        """
        rounded = tuple(round(a,digits) for a in self)
    ...
    @y.setter
    def y(self,value):
        self.values = self.values[0:1] + (value,) + self.values[2:]
    ...
    @z.setter
    def z(self,value):
        self.values = self.values[0:2] + (value,) + self.values[3:]
